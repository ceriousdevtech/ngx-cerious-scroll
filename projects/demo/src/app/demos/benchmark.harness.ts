 ...
 * deliberately measures the WRAPPER, so the numbers include Angular's own view
 * rendering. The directive commits row views synchronously inside its render
 * pass, so timing around `surface.paint()` captures that work.
 */
...
export async function collectEnv(surfaceEl: HTMLElement | null): Promise<EnvInfo> {
  ...
  return {
    ...
    wrapper: '@ceriousdevtech/ngx-cerious-scroll',
...
export function toCsv(payload: ExportPayload): string {
  const e = payload.env;
  const head = [
    '# cerious-scroll benchmark (Angular wrapper)',
...
export function toMarkdown(payload: ExportPayload): string {
  const e = payload.env;
  let out =
    `**cerious-scroll benchmark · Angular wrapper** · ${e?.browser ?? '?'} on ${e?.platform ?? '?'}` +
...
export function stamp(): string {
  return `cerious-scroll-angular-benchmark-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}`;
}
